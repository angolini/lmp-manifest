...
Commit message **subject** lines (note bitbake omits the word "layer"). The two
tagged layers append ` up to <MILESTONE>`, where `<MILESTONE>` is the target tag
with the `yocto-` prefix stripped (e.g. `yocto-5.0.18` → `5.0.18`). meta-openembedded
has no milestone tag, so its subject carries no suffix:

| Layer              | Subject                                                  |
|--------------------|----------------------------------------------------------|
| `openembedded-core`| `lmp-base: update openembedded-core layer up to <MILESTONE>` |
| `bitbake`          | `lmp-base: update bitbake up to <MILESTONE>`             |
...
lmp-base: update openembedded-core layer up to 5.0.18
...
```
lmp-base: update bitbake up to 5.0.18
